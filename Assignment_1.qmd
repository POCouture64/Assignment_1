...
summarise(avengers,avg_CE = mean(CombatEffectiveness,na.rm=TRUE)) %>% print(digits = 10)
summarise(avengers,sd_CE = sd(CombatEffectiveness,na.rm=TRUE)) %>% print(digits = 10)
range(avengers$CombatEffectiveness, na.rm = TRUE) %>% print(digits = 10)
```
...
summarise(avengers,avg_kills = mean(kills,na.rm=TRUE)) %>% print(digits = 10)
summarise(avengers,sd_kills = sd(kills,na.rm=TRUE)) %>% print(digits = 10)
...
summarise(avengers,avg_injuries = mean(injuries,na.rm=TRUE)) %>% print(digits = 10)
summarise(avengers,sd_injuries = sd(injuries,na.rm=TRUE)) %>% print(digits = 10)
...
# I chose the small effect size because I would be happy to find any effect. The significance level of .5 was used because that is what most studies in psychology use. I made the power .8 - might change this later.
pwr.t.test(d=.2, sig.level = .05, power = .8) %>% print(digits = 10)
...
powerTOSTtwo(alpha=.05, statistical_power = .8, low_eqbound_d= -.2, high_eqbound_d=.2) %>% print(digits = 10)
